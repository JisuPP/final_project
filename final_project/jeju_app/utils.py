...
def tokenize_user_input(user_input):
    okt = Okt()
    # 불용어 설정
    stopwords_path = os.path.join(settings.BASE_DIR, 'final_project', 'modules', 'ko-stopwords.csv')  # 파일 경로 수정
...
def load_tfidf_matrix(filename='tfidf_matrix.pkl'):  # 파일 경로 수정
    file_path = os.path.join(settings.BASE_DIR,  'final_project','modules', 'tfidf_matrix.pkl')
    tfidf_mat = pd.read_pickle(file_path)
    return tfidf_mat
def load_tfidf_vectorizer(filename='tfidf_vectorizer.pkl'):
    file_path = os.path.join(settings.BASE_DIR, 'final_project', 'modules', filename)
